...
This scraper extracts job listings from Garmin Workable careers page (Romania only) and imports them to peviitor.ro.

Target: https://apply.workable.com/garmin-cluj/
...
1. **Start with brand** - We know the brand (e.g., "Garmin")
...
8. **Scrape new jobs** - Extract jobs from Workable Garmin Cluj API (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Garmin Workable jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Workable Garmin Cluj API (Romania)
...
- **Workable API**: `POST https://apply.workable.com/api/v3/accounts/garmin-cluj/jobs`
