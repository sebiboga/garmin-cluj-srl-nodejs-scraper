...
describe('solr.js', () => {
  ...
  describe('querySOLR', () => {
    it('should return response object with docs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { id: 'job1', url: 'https://apply.workable.com/garmin-cluj/j/1', cif: '18850101' },
        { id: 'job2', url: 'https://apply.workable.com/garmin-cluj/j/2', cif: '18850101' }
      ]));

      const result = await solr.querySOLR('18850101');
    ...
    it('should throw when SOLR_AUTH is missing', async () => {
      delete process.env.SOLR_AUTH;
      await expect(solr.querySOLR('18850101')).rejects.toThrow('SOLR_AUTH not set in environment');
      process.env.SOLR_AUTH = 'test:test';
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('18850101')).rejects.toThrow('SOLR query error: 500');
    });
  });
  ...
  describe('queryCompanySOLR', () => {
    it('should return company data', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { id: '18850101', company: 'GARMIN CLUJ SRL', brand: 'Garmin' }
      ]));

      const result = await solr.queryCompanySOLR('id:18850101');

      expect(result.numFound).toBe(1);
      expect(result.docs[0].brand).toBe('Garmin');
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.queryCompanySOLR('id:18850101')).rejects.toThrow('SOLR company query error: 401');
    });
  });
  ...
  describe('upsertJobs', () => {
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.upsertJobs([{ url: 'https://apply.workable.com/garmin-cluj/j/bad' }])).rejects.toThrow('SOLR upsert error: 400');
    });
  ...
  describe('deleteJobByUrl', () => {
    it('should delete a job by URL', async () => {
      ...
      await expect(solr.deleteJobByUrl('https://apply.workable.com/garmin-cluj/j/old-job')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobByUrl('https://apply.workable.com/garmin-cluj/j/bad')).rejects.toThrow('SOLR delete error: 500');
    });
  });
  ...
  describe('deleteJobsByCIF', () => {
    it('should delete all jobs for a CIF', async () => {
      ...
      await expect(solr.deleteJobsByCIF('18850101')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobsByCIF('18850101')).rejects.toThrow('SOLR delete error: 500');
    });
  });
  ...
  describe('Data Integrity', () => {
    it('should not have duplicate URLs for same CIF', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://apply.workable.com/garmin-cluj/j/1', title: 'Job 1', cif: '18850101' },
        { url: 'https://apply.workable.com/garmin-cluj/j/2', title: 'Job 2', cif: '18850101' }
      ]));
    ...
    it('should have valid CIF format for all jobs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://apply.workable.com/garmin-cluj/j/1', title: 'Job 1', cif: '18850101' },
        { url: 'https://apply.workable.com/garmin-cluj/j/2', title: 'Job 2', cif: '12345678' }
      ]));
    ...
    it('should detect invalid CIF format', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { url: 'https://apply.workable.com/garmin-cluj/j/1', title: 'Job 1', cif: 'abc' }
      ]));
    ...
    it('should have valid status values', async () => {
      ...
      mockFetch.mockResolvedValue(makeSolrResponse(3, [
        { url: 'https://apply.workable.com/garmin-cluj/j/1', title: 'Job 1', cif: '18850101', status: 'scraped' },
        { url: 'https://apply.workable.com/garmin-cluj/j/2', title: 'Job 2', cif: '18850101', status: 'verified' },
        { url: 'https://apply.workable.com/garmin-cluj/j/3', title: 'Job 3', cif: '18850101', status: 'published' }
      ]));
